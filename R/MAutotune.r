...
#' @export
MAutotune <- R6::R6Class("MAutotune", public = list(VersionNumber = NULL, m_AutotuningInProgress = NULL,
    m_PercentComplete = NULL, m_AutotuningSucceeded = NULL, m_NumPatternsToAutotune = NULL,
    m_ErrorStringBuffer = NULL, m_FeaturesToTuneArray = NULL, m_NCP = NULL, m_AP = NULL,
    initialize = function(VersionNumber, m_AutotuningInProgress, m_PercentComplete,
        m_AutotuningSucceeded, m_NumPatternsToAutotune, m_ErrorStringBuffer, m_FeaturesToTuneArray,
        m_NCP, m_AP) {
        ...
        if (!missing(m_PercentComplete)) {
            stopifnot(is.numeric(m_PercentComplete), length(m_PercentComplete) ==
                1)
        ...
        if (!missing(m_NumPatternsToAutotune)) {
            stopifnot(is.numeric(m_NumPatternsToAutotune), length(m_NumPatternsToAutotune) ==
                1)
            self$m_NumPatternsToAutotune <- m_NumPatternsToAutotune
        }
        if (!missing(m_ErrorStringBuffer)) {
            stopifnot(is.character(m_ErrorStringBuffer), length(m_ErrorStringBuffer) ==
                1)
        ...
        if (!is.null(MAutotuneObject$VersionNumber)) {
            ...
            VersionNumberObject$fromJSON(jsonlite::toJSON(MAutotuneObject$VersionNumber,
                auto_unbox = TRUE))
        ...
        if (!is.null(MAutotuneObject$m_PercentComplete)) {
            ...
            m_PercentCompleteObject$fromJSON(jsonlite::toJSON(MAutotuneObject$m_PercentComplete,
                auto_unbox = TRUE))
        ...
        sprintf("{
           ...
           \"m_AP\": %s
            ...
            self$VersionNumber$toJSON(), self$m_AutotuningInProgress, self$m_PercentComplete$toJSON(),
            self$m_AutotuningSucceeded, self$m_NumPatternsToAutotune, self$m_ErrorStringBuffer,
            lapply(self$m_FeaturesToTuneArray, function(x) paste(paste0("\"", x,
                "\""), sep = ",")), self$m_NCP$toJSON(), self$m_AP$toJSON())
    }, fromJSONString = function(MAutotuneJson) {
        MAutotuneObject <- jsonlite::fromJSON(MAutotuneJson)
        VersionNumberObject <- VersionNumber$new()
        self$VersionNumber <- VersionNumberObject$fromJSON(jsonlite::toJSON(MAutotuneObject$VersionNumber,
            auto_unbox = TRUE))
        self$m_AutotuningInProgress <- MAutotuneObject$m_AutotuningInProgress
        BigDecimalObject <- BigDecimal$new()
        self$m_PercentComplete <- BigDecimalObject$fromJSON(jsonlite::toJSON(MAutotuneObject$m_PercentComplete,
            auto_unbox = TRUE))
        self$m_AutotuningSucceeded <- MAutotuneObject$m_AutotuningSucceeded
        self$m_NumPatternsToAutotune <- MAutotuneObject$m_NumPatternsToAutotune
        self$m_ErrorStringBuffer <- MAutotuneObject$m_ErrorStringBuffer
        self$m_FeaturesToTuneArray <- MAutotuneObject$m_FeaturesToTuneArray
        MNCPObject <- MNCP$new()
        self$m_NCP <- MNCPObject$fromJSON(jsonlite::toJSON(MAutotuneObject$m_NCP,
            auto_unbox = TRUE))
        MAPObject <- MAP$new()
