...
#' @export
PutConfigResponse <- R6::R6Class("PutConfigResponse", public = list(features = NULL,
    streaming = NULL, initialize = function(features, streaming) {
        ...
        if (!is.null(PutConfigResponseObject$streaming)) {
            ...
            streamingObject$fromJSON(jsonlite::toJSON(PutConfigResponseObject$streaming,
                auto_unbox = TRUE))
        ...
        PutConfigResponseObject <- jsonlite::fromJSON(PutConfigResponseJson)
        self$features <- lapply(PutConfigResponseObject$features, function(x) FusionConfig$new()$fromJSON(jsonlite::toJSON(x,
            auto_unbox = TRUE)))
        StreamingParametersObject <- StreamingParameters$new()
        self$streaming <- StreamingParametersObject$fromJSON(jsonlite::toJSON(PutConfigResponseObject$streaming,
            auto_unbox = TRUE))
    }))
